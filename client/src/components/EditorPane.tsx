...
export function EditorPane() {
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-[#0d0e12]">
      <div className="flex h-10 min-h-10 shrink-0 items-stretch overflow-x-auto border-b border-white/[0.06] bg-[#101116]">
        {openTabs.length === 0 ? (
          <div className="flex items-center gap-2 px-4 text-[11px] text-zinc-600">
            <FileCode2 size={14} />
          ...
          openTabs.map((tab) => {
            ...
            return (
              <button
                key={tab.id}
                type="button"
                className={`group flex min-w-[130px] items-center gap-2 border-r border-white/[0.06] px-3 text-left text-[12px] ${
                  tab.id === activeTabId
                    ? "border-t-2 border-t-blue-400 bg-[#0d0e12] text-zinc-200"
      ...
      {activeFile && activeFile.kind === "file" ? (
        <div ref={editorHostRef} className="relative min-h-0 min-w-0 flex-1 overflow-hidden bg-[#0d0e12]">
          <Editor
            height="100%"
            width="100%"
        ...
        <div className="flex min-h-0 min-w-0 flex-1 items-center justify-center text-center">
          <div className="max-w-xs">
            <div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl border border-white/[0.08] bg-white/[0.03] text-blue-300">
